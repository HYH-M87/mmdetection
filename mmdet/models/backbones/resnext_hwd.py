...
class HWDResNeXt(ResNet):
    ...
    def __init__(self, groups=1, base_width=4, **kwargs):
        ...
        super(HWDResNeXt, self).__init__(strides=(1,2,2,2),**kwargs)
        self.pool1 = Down_wt(64,64)
        self.up = torch.nn.Conv2d(64,256,1)
        self.pool2 = Down_wt(256,512)
        self.pool3 = Down_wt(512,1024)
        self.pool4 = Down_wt(1024,2048)
        self.pools = [None,self.pool2,self.pool3,self.pool4 ]
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.res_layers): 
            ...
            if(i==0):
                append = self.up(outs[i])
            else:
                append = self.pools[i](outs[i])
            x = res_layer(x)
            x = x + append
